...
public:
  ...
  void set_uniform(std::string_view uniform_name, texture_gl_es20 *texture) {
    ...
    GLint location = glGetUniformLocation(program_id, uniform_name.data());
    if (location == -1) {
...
quad::quad(const float &sc)
    : dir(direction::up), position(vec2(0, 0)), scale(sc), angle(0) {
  fill_vert_coord();
  fill_text_coord();
}
...
void quad::move_up() {
  ...
  if (position.y < 2 - 2 * scale && dir == direction::up)
    position.y += 2 * scale;
...
void quad::move_down() {
  ...
  if (position.y >= 2 * scale && dir == direction::down)
    position.y -= 2 * scale;
  angle = M_PI;
  dir = direction::down;
}
...
void quad::move_left() {
  if (position.x >= 2 * scale && dir == direction::left)
    position.x -= 2 * scale;
  angle = -M_PI_2;
  dir = direction::left;
}

void quad::move_right() {

  if (position.x < 2 - 2 * scale && dir == direction::right)
    position.x += 2 * scale;
  angle = M_PI_2;
  dir = direction::right;
}
